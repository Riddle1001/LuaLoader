-- Scraped by chicken
-- Author: Sl1tex
-- Title [Release] [FIX] Anti-aim circle with indicators 
-- Forum link https://aimware.net/forum/thread/144299
...
local function circle()
  ...
  if lp ~= nil and gui.GetValue("rbot.master") then
    ...
    if attind:GetValue() then
      offset = offset + 1
      if gui.GetValue("rbot.antiaim.advanced.autodir") ~= 0 then
        draw.Color(ir, ig, ib, ia)
      else
        draw.Color(bgr, bgg, bgb, bga)
      end
      draw.TextShadow(x - draw.GetTextSize("AUTODIR")/2, y + radius + offset * 18, "AUTRODIR")
    end
