...
local Factor_test = gui.Slider(DT_REF, "Factor_test", "Factor", 17, 3, 17)
local Factor_Triggor = gui.Slider(DT_REF, "Factor_Triggor", "Factor Triggor", 3, 3, 17)
local F_key = gui.Keybox(DT_REF, "factor_key", "Factor Triggor key (Set to None for auto Mode)", 0)
...
if LocalPlayer and F_key:GetValue() == 0 or (F_key:GetValue() and input.IsButtonDown(F_key:GetValue())) then
...
elseif testget:GetValue() == Factor_Triggor:GetValue() and not Triggor and #players == 0 then --idk if this work lol
  ui.SetValue( "misc.fakelag.factor", Factor_test:GetValue() ) 
end
